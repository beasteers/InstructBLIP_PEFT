...
from lavis.models.blip2_models.cls_head import CLS_HEAD

@registry.register_model("blip2_t5_instruct_qformer_lora_clsf")
class Blip2T5InstructQformerLoRAClsf(Blip2Base):
    """
    BLIP2 T5 model.
...
LORA DISABLED
    ...
    def _encode_vision(self, image):
        with self.maybe_autocast():
            if image.ndim == 5:
                B, T, C, H, W = image.size()  # 64, 6, 3, 224, 224
                image = image.reshape(B*T, C, H, W) # 64*6, 3, 224, 224
                image_embeds = self.ln_vision(self.visual_encoder(image))
                _, L, C = image_embeds.size()  # 64*6, 32, 768
                image_embeds = image_embeds.reshape(B, T*L, C) # 64, 6*32, 768
            else:
                image_embeds = self.ln_vision(self.visual_encoder(image))
    ...
    def generate(
        ...
        if image.dim() == 6:
            ...
            for j in range(image.size(1)):
                this_frame = image[:,j]
                frame_embeds, frame_atts = self._encode_vision(this_frame)
    ...
    def from_config(cls, cfg):
        ...
        qformer_cls_arch = cfg.get("qformer_cls_arch", None)
        qformer_cls_loss_weight = cfg.get("qformer_cls_loss_weight", 1)
        ...
        with lora(r, alpha, dropout, enabled=self_attention_qv_lora, qkv=[qformer_crossattention_lora_q, qformer_crossattention_lora_k, qformer_crossattention_lora_v]), \
            ...
            model = cls(
                ...
                qformer_cls_arch=qformer_cls_arch,
                qformer_cls_loss_weight=qformer_cls_loss_weight,
